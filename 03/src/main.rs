...
fn main() {
    ...
    visited_one.reserve(300);

    let (mut wox, mut woy) = (0i64, 0i64);

    for s in wire_one_str.split(",") {
        let st = s.to_string();
        let c: i64 = String::from(&st[1..]).parse().unwrap();
        match &st[0..1] {
            "L" => {
                for x in wox-c..wox {
    ...
    wox = 0;
    woy = 0;
    for s in wire_two_str.split(",") {
        let st = s.to_string();
        let c: i64 = String::from(&st[1..]).parse().unwrap();
        match &st[0..1] {
            "L" => {
                for x in wox-c..wox {
                    if manhattan(x, woy) < min_dist && visited_one.contains(&(x, woy)) {
                        min_dist = manhattan(x, woy);
                    }
                }
                wox = wox - c;
            }
            ...
            "D" => {
                for y in woy-c..woy {
                    if manhattan(wox, y) < min_dist && visited_one.contains(&(wox, y)) {
                        min_dist = manhattan(wox, y);
                    }
                }
                woy = woy - c;
            }
