...
fn parse_str(thestr: String) -> Vec<(Direction, i64)> {
    let mut v: Vec<(Direction, i64)> = vec![];

    for s in thestr.split(",") {
        let st = s.to_string();
        let d: Direction = match &st[0..1] {
            "L" => Direction::L,
            "R" => Direction::R,
            "D" => Direction::D,
            "U" => Direction::U,
        ...
        let i: i64 = String::from(&st[1..]).parse().unwrap();
        v.push((d, i));
    }
...
fn main() {
    ...
    let mut min_dist = 99999i64;
    let (mut wtx, mut wty) = (0i64, 0i64);
    for (d, c) in wire_two {
        match d {
            Direction::L => {
                for x in wtx-c..wtx {
                    if visited_one.contains(&(x, wty)) {
                        if manhattan(x, wty) < min_dist {
                            min_dist = manhattan(x, wty);
                        }
                    }
                }
                wtx = wtx - c;
            ...
            Direction::D => {
                for y in wty-c..wty {
                    if visited_one.contains(&(wtx, y)) {
                        if manhattan(wtx, y) < min_dist {
                            min_dist = manhattan(wtx, y);
                        }
                    }
                }
                wty = wty - c;
            ...
            Direction::U => {
                for y in wty..wty+c {
                    if visited_one.contains(&(wtx, y)) {
                        if manhattan(wtx, y) < min_dist && manhattan(wtx, y) != 0 {
                            min_dist = manhattan(wtx, y);
                        }
                    }
                }
